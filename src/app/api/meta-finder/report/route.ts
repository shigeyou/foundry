...
type ScopeType = "company-wide" | "executive" | "cross-cutting" | "indirect" | "department";

function getScopeType(scopeId: string): ScopeType {
  if (scopeId === "all") return "company-wide";
  if (scopeId === "eo") return "executive";
  if (["cc-plus", "dx-cocreation", "diversity"].includes(scopeId)) return "cross-cutting";
  if (["planning", "hr", "finance"].includes(scopeId)) return "indirect";
  return "department";
}

const RAG_CHAR_LIMITS: Record<ScopeType, { engagement: number; strategic: number; budget: number }> = {
  "company-wide":  { engagement: 8000,  strategic: 8000,  budget: 8000  },
  "executive":     { engagement: 4000,  strategic: 15000, budget: 12000 },
  "cross-cutting": { engagement: 8000,  strategic: 12000, budget: 8000  },
  "indirect":      { engagement: 12000, strategic: 12000, budget: 8000  },
  "department":    { engagement: 15000, strategic: 15000, budget: 12000 },
};
...
async function generateScopeReport(
  ...
  try {
    ...
    // スコープ種別判定
    const scopeType = getScopeType(scope.id);
    const isCompanyWide = scopeType === "company-wide";
    const isExecutive = scopeType === "executive";

    // アイデアをフォーマット（全社向けも切り詰めなし — description + reason のフル情報を提供）
...
${evidence.length > 0 ? `根拠引用: ${evidence.map((e: string) => `「${e}」`).join("")}` : ""}`;
    ...
    // 財務データを取得
    const deptFinancial = getDeptFinancial(isExecutive ? "all" : scope.id);
...
**「実績」ではなく「予算・見込み」であることを明記し、赤字予算の部門は「黒字化・損益改善」を主軸に、黒字予算の部門は「利益拡大・競争力強化」を主軸に分析してください。**\n${isExecutive ? `\n**注：これは全社連結P/Lデータです。執行役員として全社の損益責任を負う立場から分析してください。赤字3事業部（海洋技術・シミュレータ技術・オンサイト）の個別対処にも言及すること。※洋上風力部はFY25末で廃止済み。**\n` : ""}`;
...
${deptFinancial.budgetDeptName}は間接部門のため個別P/Lはありません。\n`;
    ...
    const engagementChunks = await retrieveRelevantChunks({
      ...
      docTypes: ["survey"],
      topK: 15,
      maxChars: RAG_CHAR_LIMITS[scopeType].engagement,
    });
    ...
    const strategicChunks = await retrieveRelevantChunks({
      ...
      topK: 15,
      maxChars: RAG_CHAR_LIMITS[scopeType].strategic,
    });
    ...
    const budgetChunks = await retrieveRelevantChunks({
      ...
      docTypes: ["budget"],
      topK: 15,
      maxChars: RAG_CHAR_LIMITS[scopeType].budget,
    });
    const budgetContext = formatChunksForPrompt(budgetChunks, "予算・財務関連データ");

    const prompt = `あなたは${isCompanyWide ? "CDIO直轄の経営戦略アドバイザー" : isExecutive ? "取締役会直属の経営分析アドバイザー" : "経営企画部の分析担当"}です。メタファインダーで発見されたアイデア群を分析し、${scope.name}の経営レポートを日本語で作成してください。
...
- 以下の表現は**結論を曖昧にするため禁止**：
  ...
  ❌ 「〜を推進する」「〜を強化する」（何をどう変えるのか不明）
  ❌ 「〜が求められる」「〜が重要である」「〜が不可欠である」（結論の先送り）
  → 代わりに「〜を導入する」「〜を廃止する」「〜を○○に変更する」など、行動が特定できる表現を使う
- **根拠のない因果関係の主張は禁止**（例：「エンゲージメント向上→生産性向上」は根拠データなしに書かない）
- **アイデア名をそのままevidenceとして引用する循環論法は禁止**（アイデアの存在自体は根拠にならない）
...
` : ""}${isExecutive ? `
## 【重要】執行役員レポートの視座
これは部門改善レポートではない。**執行役員として経営責任を果たすための意思決定支援レポート**である。
以下の視座を厳守すること：
- 株主・親会社（商船三井）への説明責任を果たせる内容
- 中期経営計画の進捗と達成見通し
- 全社P/Lの構造分析（赤字3事業部の個別対処を含む）
- 重大経営判断（M&A、事業撤退、大型投資）の要否
- 最大リスクとその対策
- 後継者育成・経営人材パイプライン
- **部門単位の業務改善（DX化・マニュアル整備・研修実施等）は記載しない**
` : ""}
## 対象スコープ: ${scope.name}
...
4. 勝ち筋は「どの事業にどれだけ経営資源を投下するか」レベルの意思決定を伴うものとすること` : isExecutive ? `1. 経営責任・成果責任の観点で分析すること。個別部門の業務改善ではなく、執行役員としての意思決定事項に焦点を当てる
2. 全社P/Lを最優先で分析し、赤字事業の構造的要因と対処方針を明確にすること
3. 内部課題（エンゲージメント等）は全社制度・構造レベルの問題に限定し、部門個別の運用課題は含めない
4. 勝ち筋は「経営判断を伴う戦略的意思決定」レベルとし、業務改善レベルの施策は含めない` : `1. ${scope.name}固有の業務特性・人員構成・働き方を踏まえた分析とすること
...
{${deptFinancial && deptFinancial.profitStatus !== "na" ? `
  ...
  "executiveSummary": "${isExecutive
    ? `以下の5項目を箇条書き形式で記述すること（各行は「・ラベル：内容」の形式）:\\n・全社業績評価：FY26予算の全社営業損益${deptFinancial ? formatProfitLoss(deptFinancial.fy26OperatingProfit) : ""}（予算値）。赤字事業部の状況と全社損益構造を1文で\\n・経営判断事項：執行役員として意思決定すべき最重要事項を1-2文で\\n・最大リスク：経営上の最大リスクとその影響度を1文で\\n・エンゲージメント経営課題：全社的なエンゲージメント危機の経営インパクトを1文で\\n・実行すべき意思決定：今期中に決裁すべき具体的な経営判断を1文で`
    : deptFinancial && deptFinancial.profitStatus !== "na"
    ...
    "items": [
      {
        ...
        "rationale": "この戦略が有効である根拠を2-3文で。**『なぜこの会社にしかできないのか』を必ず含める。他社でもできることは勝ち筋ではない**",
        "antiTest": "この施策を競合他社のコンサルが提案しても同じ内容になるか？→ YESなら書き直し",
...
## 制約
- ${isExecutive
    ? `executiveSummaryは「・全社業績評価」「・経営判断事項」「・最大リスク」「・エンゲージメント経営課題」「・実行すべき意思決定」の5項目で構成すること\n- financialAssessmentは全社P/Lの具体的な数字と赤字事業部の個別分析を含めること`
    : deptFinancial && deptFinancial.profitStatus !== "na"
...
- strategiesは6-10件。事業ポートフォリオの再構築、M&A/アライアンス、組織再編等の大きな打ち手を含めること。BSCスコアは1-5の整数` : isExecutive ? `- issuesは7-9件。**内部課題（エンゲージメント・組織体制等）は3割以下**とし、事業戦略・経営判断・財務構造の課題を主体とすること
- エンゲージメントは全社制度・構造レベルの課題がある場合のみ含める（部門個別の運用課題は不可）
- severity=highを4-5件含めること
- solutionsは各課題に対応して7-9件。**執行役員会議決裁レベルの施策のみ**とすること
- strategiesは5-8件。全社P/L改善・事業ポートフォリオ再編・M&A等の経営判断を伴う打ち手を含めること。BSCスコアは1-5の整数` : scopeType === "cross-cutting" ? `- issuesは6-8件。**${scope.name}のテーマに即した全社横断の課題を中心に**構成すること
- エンゲージメントを含める場合は**${scope.name}のテーマと関連付けて**記述すること（テーマと無関係なエンゲージメント一般論は不可）
- severity=highを3-4件含めること
- solutionsは各課題に対応して6-8件
- strategiesは5-8件、BSCスコアは1-5の整数` : `- issuesは6-8件。うち半数は組織・人材・マネジメント・エンゲージメント等の内部課題とすること
