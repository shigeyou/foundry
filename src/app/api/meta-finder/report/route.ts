...
{
  "executiveSummary": "4-5文で要約。最初に${scope.name}の部門特徴（役割・強み・人員の働き方）を1文で述べ、次にこの部門固有のエンゲージメント上の状況（サーベイデータがあればこの部門のスコアを引用、なければ業務特性から推論した固有リスク）に触れ、事業課題と組織内部課題の両面から主要な問題点と最も有望な戦略をまとめる。",
  "issues": {
    "items": [
      {
...
## 重要な制約
- executiveSummaryは必ず部門特徴への言及から始め、**この部門固有の**エンゲージメント状況にも触れること
